...
describe('formatDate', () => {
  ...
  it('dice "hoy" para el mismo dia calendario, sin importar la hora', () => {
    expect(formatDate('2026-08-15T23:59:00', now)).toBe('hoy');
  });

  it('usa la fecha larga desde el dia anterior, aunque hayan pasado pocas horas', () => {
    expect(formatDate('2026-08-14T23:00:00', now)).toBe('14 Agosto 2026');
  });

  it('usa la fecha larga para cualquier fecha mas antigua', () => {
    expect(formatDate('2026-01-01T14:05:00', now)).toBe('1 Enero 2026');
  });
