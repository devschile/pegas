...
CREATE TABLE public.ads (
    ...
    CONSTRAINT ads_altos_positivos CHECK ((((alto_desktop IS NULL) OR ((alto_desktop >= 1) AND (alto_desktop <= 200))) AND ((alto_movil IS NULL) OR ((alto_movil >= 1) AND (alto_movil <= 200))))),
...
CREATE TABLE public.pegas_similares (
    pega_id integer NOT NULL,
    similar_id integer NOT NULL,
    score real NOT NULL,
    motivo text NOT NULL,
    calculado_en timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT pegas_similares_no_autorreferencia CHECK ((pega_id <> similar_id)),
    CONSTRAINT pegas_similares_score_normalizado CHECK (((score >= (0)::double precision) AND (score <= (1)::double precision)))
);

CREATE TABLE public.relacionadas_eventos (
    id bigint NOT NULL,
    pega_id integer NOT NULL,
    similar_id integer NOT NULL,
    tipo text NOT NULL,
    ...
    dispositivo text NOT NULL,
    fecha timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT relacionadas_eventos_dispositivo_check CHECK ((dispositivo = ANY (ARRAY['desktop'::text, 'movil'::text]))),
    CONSTRAINT relacionadas_eventos_tipo_check CHECK ((tipo = ANY (ARRAY['impresion'::text, 'click'::text])))
);

ALTER TABLE public.relacionadas_eventos ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.relacionadas_eventos_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE ONLY public.pegas_similares
    ADD CONSTRAINT pegas_similares_pkey PRIMARY KEY (pega_id, similar_id);

ALTER TABLE ONLY public.pegas
    ADD CONSTRAINT pegas_url_key UNIQUE (url);

ALTER TABLE ONLY public.relacionadas_eventos
    ADD CONSTRAINT relacionadas_eventos_pkey PRIMARY KEY (id);
...
CREATE INDEX idx_pegas_similares_pega ON public.pegas_similares USING btree (pega_id, score DESC);

CREATE INDEX idx_relacionadas_eventos_motivo ON public.relacionadas_eventos USING btree (motivo, tipo);

CREATE INDEX idx_relacionadas_eventos_similar ON public.relacionadas_eventos USING btree (similar_id, fecha DESC);
...
ALTER TABLE ONLY public.pegas_similares
    ADD CONSTRAINT pegas_similares_pega_id_fkey FOREIGN KEY (pega_id) REFERENCES public.pegas(id) ON DELETE CASCADE;

ALTER TABLE ONLY public.pegas_similares
    ADD CONSTRAINT pegas_similares_similar_id_fkey FOREIGN KEY (similar_id) REFERENCES public.pegas(id) ON DELETE CASCADE;
