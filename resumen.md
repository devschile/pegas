...
### Evaluación 2026-08-28: ocho candidatas más (ninguna se integra)

- **trabajoremotochile.com** — la única con API pública real: `GET /api/jobs?limit=100&page=N`, sin auth, JSON rico (título, empresa, ubicación, `salary_min/max/currency`, tags, `apply_url`, `created_at`, incluso `title_es`/`description_es` traducidos). Se descarga entero en 8 llamadas: **798 avisos**. **Pero es un espejo de nuestras propias fuentes**: el campo `source` dice weworkremotely 419, jobicy 167, getonbrd 127, himalayas 73, remotive 11, manual 1. Contenido propio: **un aviso**. De los 798, 470 son de TI y solo **102 traen señal LatAm/Chile** — y esos 102 son jobicy (71), himalayas (29) y remotive (2), o sea justo las fuentes que ya evaluamos y que en el caso de Jobicy e Himalayas ya están construidas como nodos del workflow. Encima traduce los títulos con IA (`ai_confidence`, `ai_reason`), así que llegaríamos a mostrar "Ingeniero de Soluciones AI Senior (Python & AWS)" en vez del título original. **No aporta nada que no podamos sacar de la fuente directa.**
- **beBee** (`bebee.com/cl/jobs`): reconfirmado el `Disallow: /api/` de julio. Sin cambios.
- **Jooble** (`cl.jooble.org`): el `robots.txt` prohíbe `/SearchResult`, y la API sin key devuelve Cloudflare 403. Tiene API oficial gratuita pero **exige registrarse para obtener una key** — es la única de las ocho que podría revivir si alguien saca esa key.
- **NTT** (`nttlimited.wd3.myworkdayjobs.com`): la API CXS de Workday **sí es pública** — `POST /wday/cxs/nttlimited/NTT_Careers/jobs` con `{"appliedFacets":{"locations":["5093bb967041101c289b05d1d95218ef"]},"limit":20,"offset":0}` devuelve JSON. Con ese filtro son **12 avisos en Santiago**, casi todos de soporte, redes y sysadmin. Es una sola empresa, mismo criterio con el que se descartaron AcidLabs y OPTION. Lo que sí queda anotado es el **patrón**: cualquier empresa con Workday expone ese mismo endpoint, así que una fuente "varios tenants de Workday en Chile" es viable si algún día se quiere volumen corporativo.
- **expertini** (`santiago.cl.expertini.com`), **recruit.net** (`global.recruit.net`) y **Glassdoor**: Cloudflare 403 en todo, incluidos `/rss` y `/sitemap.xml`. Glassdoor además tiene `Disallow: /search/` y `/rss/*` en su `robots.txt`. Mismo caso que it-hunter.cl.
- **levels.fyi**: la página de Chile responde 200 y trae `__NEXT_DATA__` legible, pero adentro hay **2 avisos**. Sin volumen no hay fuente.

**Conclusión de la ronda:** ninguna se integra. El hallazgo útil es indirecto — que trabajoremotochile revenda ~100 avisos LatAm de Jobicy e Himalayas confirma que esas dos fuentes sí tienen contenido aprovechable para Chile, y ya están implementadas en nuestro workflow: sus nodos existen pero quedaron huérfanos (ningún trigger los alimenta) y con una copia vieja de `categorizar()`. Reconectarlas es más barato y da mejores datos que consumir un intermediario que traduce los títulos con IA.
